...
(function ($, window) {
    ...
    var mycenter = {
        ...
        Event_addBlockQ: function (th) {
            var tpl = '<blockquote class="cp_block J_blockQ clear" style="display:none;cursor:move;">' + '<div class="left addicon J_fileImag" >' + '<input type="hidden" value="" name="step_img[]" class="J_imghidden" />' + '<input type="file" name="file"  value=""/>' + '<p class="p1">点击上传步骤图</p>' + '<p class="p3">（必填）</p>' + "</div>" + '<div class="left">' + '<input type="hidden" name="stepid[]" value="">' + '<textArea name="note[]" class="textArea J_input" placeholder="请输入做法说明菜谱描述"></textArea>' + '<span class="J_step_num"></span>' + '<a href="javascript:;" class="add J_addTextarea"></a><a class="up J_upTextarea" href="javascript:;"></a><a class="down J_downTextarea" href="javascript:;"></a><a href="javascript:;" class="delete J_delTextarea"></a>' + "</div>" + "</blockquote>";
